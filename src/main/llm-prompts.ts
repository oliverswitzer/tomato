...
export function buildBatchPrompt(options: BatchPromptOptions): string {
  const { timeline, intention, sessionContext } = options;
  ...
  const passiveContextSections = buildPassiveContextSection(timeline);
...
export function buildSessionPrompt(
  ...
  return `You are summarizing a completed pomodoro focus session.
...
function buildPassiveContextSection(timeline: ActivityTimeline): string {
...
export function parseBatchResponse(text: string): ParsedBatchResponse | null {
  try {
...
export function parseSessionResponse(text: string): ParsedSessionResponse | null {
  try {
    const jsonMatch = text.match(/\{[\s\S]*\}/);
    if (!jsonMatch) return null;
