...
export function StartPage() {
  ...
  return (
    <div className="bg-cream rounded-3xl px-12 pt-9 pb-8 flex flex-col items-center gap-6 h-screen overflow-y-auto [&::-webkit-scrollbar]:w-0">
      ...
      <div className="w-20 h-20 rounded-3xl flex items-center justify-center text-5xl drop-shadow-[0_4px_12px_rgba(226,87,76,0.15)]">
        🍅
      </div>

      <div className="text-center flex flex-col gap-1.5">
        <div className="font-serif text-3xl font-medium text-text tracking-tight">
      ...
      <div className="[-webkit-app-region:no-drag] w-full flex flex-col gap-3">
        <div className="relative w-full">
          <textarea
            ...
            className="bg-white border-[1.5px] border-accent/20 rounded-2xl px-5 pt-5 pb-7 font-serif text-lg text-text outline-none w-full shadow-[0_2px_12px_rgba(42,42,42,0.05)] transition-colors duration-200 resize-none leading-snug placeholder:text-subtle focus:border-accent/50"
        ...
        <div className="flex gap-2 flex-wrap">
          {recentChips.length > 0
            ? recentChips.map((text, i) => (
                <div
                  key={text}
                  className="flex items-center gap-1.5 px-3.5 py-2 bg-white border border-border rounded-full text-sm font-medium text-text cursor-pointer transition-all duration-150 hover:bg-[#F7F2EA] hover:border-accent/20"
      ...
      <div className="[-webkit-app-region:no-drag] w-full flex flex-col gap-2.5">
        <div className="flex justify-between items-center">
          <span className="text-sm font-semibold text-text">Session length</span>
        </div>
        <div className="flex gap-2.5">
          {TIMER_OPTIONS.map((opt) => (
            <div
              key={opt.min}
              className={`flex-1 flex flex-col items-center gap-0.5 py-3.5 px-3 rounded-2xl border cursor-pointer transition-all duration-200 ${
            ...
            >
              ...
              <span className={`text-xs ${selectedMinutes === opt.min ? 'text-[#FBE5E3] font-semibold' : 'text-muted font-medium'}`}>
                {opt.label}
              </span>
            </div>
          ))}
      ...
      <div className="[-webkit-app-region:no-drag] w-full flex flex-col items-center gap-3">
        {!hasScreenPermission && (
          <div className="w-full px-4 py-3 bg-[#FEE4E2] rounded-xl text-sm text-[#B42318] leading-normal">
      ...
      {recentSessions.length > 0 && (
        <div className="[-webkit-app-region:no-drag] w-full flex flex-col gap-3">
          <div className="flex justify-between items-center">
            <span className="text-sm font-semibold text-text">Recent sessions</span>
            ...
            {recentSessions.map((s, i) => (
              ...
              >
                <div className="flex justify-between items-start gap-2">
                  <span className="flex items-start gap-1.5 text-sm font-semibold text-text flex-1 min-w-0">
                    <span
                ...
                {s.summary && (
                  <div className="text-xs text-[#6B6259] leading-normal">{s.summary}</div>
                )}
              </Card>
            ))}
          </div>
